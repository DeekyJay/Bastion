...
exports.exec = async (Bastion, message, args) => {
  ...
  if (user.presence.game) {
    activity = `${Bastion.Constants.ActivityTypes[user.presence.game.type]} ${
  ...
  await message.channel.send({
    embed: {
      ...
      footer: {
        text: `${
          message.guild.ownerID === user.id ? 'Server Owner •' : ''
        } Shares ${mutualGuilds} servers with me.`,
...
exports.config = {
  aliases: ['uinfo'],
  enabled: true,
  argsDefinitions: [{ name: 'id', type: String, defaultOption: true }],
};
