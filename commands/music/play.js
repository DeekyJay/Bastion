...
exports.exec = async (Bastion, message, args) => {
  try {
    if (
      message.guild.music.textChannelID &&
      message.channel.id !== message.guild.music.textChannelID
    )
      return Bastion.log.info(
        'Music channels have been set, so music commands will only work in the music text channel.',
      );
    ...
    let channels = Bastion.channels.filter(
    ...
    if (!channels) {
      return Bastion.emit(
        'error',
        'No Voice Channel',
        'There is nobody in a voice channel.',
        message.channel,
      );
    }
    ...
    if (textChannel.id !== message.channel.id) {
      return Bastion.log.info(
        `Music commands will only work in the ${textChannel.name} text channel for this session.`,
      );
    }
    ...
    if (args.playlist) {
      ...
      if (!playlistModel || !playlistModel.dataValues.songs.length) {
        return Bastion.emit(
          'error',
          '',
          Bastion.i18n.error(message.guild.language, 'playlistNotFound'),
          message.channel,
        );
      }
      ...
      if (
        /^http[s]?:[/]{2}(?:www\.)?youtube\.com\/(?:(?:playlist|watch)\?(?:[a-z0-9-_=&.]{1,2021})?list=[a-z0-9-_]{1,64}(?:[a-z0-9-_=&.]{1,2021})?)$/i.test(
          args.song,
        )
      ) {
        // YouTube Playlist
      ...
      let songURL = /^http[s]?:[/]{2}(?:[a-z0-9](?:[a-zA-Z0-9-]{0,249}(?:[a-zA-Z0-9]))?\.?){1,127}[-a-z0-9@:%_+.~#?&/=]{0,2045}$/i.test(
        args.song,
      )
        ? args.song
        : `ytsearch:${args.song}`;
      ...
      let songInfo = await getSongInfo(
        songURL,
        playlist
          ? ['--flat-playlist', '--yes-playlist'].concat(youtubeDLOptions)
          : youtubeDLOptions,
      );
      ...
      if (!songInfo || (!playlist && (!songInfo.format_id || songInfo.format_id.startsWith('0')))) {
        ...
        return Bastion.emit(
          'error',
          '',
          Bastion.i18n.error(message.guild.language, 'notFound', 'result'),
          message.channel,
        );
      }
          ...
          .send({
            embed: {
              ...
              footer: {
                ...
                } • Duration: ${songInfo.duration || 'N/A'} • Requester: ${message.author.tag}`,
              },
            },
          })
          .catch(e => {
...
exports.help = {
  name: 'play',
  description:
    'Plays a song or adds the song to the current music queue if Bastion is already playing in your Discord server, specified either by the name, or the link, or from your favorite songs list, or from a YouTube playlist link.',
  botPermission: '',
  userTextPermission: '',
  userVoicePermission: '',
  ...
  example: [
    'play Shape of you',
    'play https://www.youtube.com/watch?v=fNVUTgd4pio',
    'play --playlist',
  ],
};
...
function startStreamDispatcher(guild, connection) {
  if (!guild.music.songs[0] || connection.channel.members.size <= 1) {
    if (guild.client.configurations.music && guild.client.configurations.music.status) {
      guild.client.user.setActivity(
        typeof guild.client.configurations.game.name === 'string'
          ? guild.client.configurations.game.name
          : guild.client.configurations.game.name.length
          ? guild.client.configurations.game.name[0]
          : null,
        {
          type: guild.client.configurations.game.type,
          url:
            guild.client.configurations.game.url &&
            guild.client.configurations.game.url.trim().length
              ? guild.client.configurations.game.url
              : null,
        },
      );
    ...
    //   });
    setTimeout(() => {
      if (!guild.music.songs[0] && guild.music.voiceChannel) {
        guild.music.dispatcher.end();
        guild.music.voiceChannel.leave();
      }
    }, 30000);
  ...
  guild.music.textChannel
    .send({
      embed: {
        ...
        footer: {
          text: `🔉 ${guild.music.dispatcher.volume * 50}% • Duration: ${guild.music.songs[0]
            .duration || 'N/A'} • Requester: ${guild.music.songs[0].requester}`,
        },
      },
    })
    .catch(e => {
